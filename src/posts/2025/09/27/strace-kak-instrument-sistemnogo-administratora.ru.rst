...
анализом многопоточных приложений, отладкой bash-скриптов и сетевых приложений. Материал ориентирован на системных администраторов,
...
Для этого мы запустим команду с флагом `-Z`, что бы увидеть только не успешные системные вызовы
...
По последним строкам вывода мы можем предположить, что наше приложение падает, из-за отсутствия доступа к файлу `/root/test`. 
...
Уже на протяжении нескольких десятков лет, никого не удивить многоядерными CPU, и для полной утилизации CPU применяется
...
ожидание выполнения `wait4(8644,... <https://man7.org/linux/man-pages/man2/wait4.2.html>`_
...
Никакой неожиданности, как и ожидалось был вызван системный вызов `vfork`, который более эффективно работает чем `fork`, 
за счет отсутствия копирования страниц памяти. Вместо этого потомок и родитель делят память до тех пор, пока один из них
...
номер PID нового процесса. Вызов `clone3` очень похож на вызов `fork`, то есть создается отдельный процесс, 
но позволяет контролировать этапы создания процесса, на основе переданных `flags`

По переданным флагам мы можем сразу увидеть
...
или с применением `debug` режима, проанализировать скрипт на 4500 строк, как например 
...
Сложно с первого взгляда определить что же все таки будет выполнять данный скрипт, давайте пропустим его выполнение через
...
Если добавим еще и ключ `-v` то получим результат еще и с переданными аргументами.
...
* `listen` - Перевели созданный сокет в режим ожидания входящих сообщений
* `accept4` - Извлекаем первый запрос на подключение из очереди ожидающих подключений для прослушиваемого сокета и создаём новый подключенный сокет. В параметрах данного вызова у нас будет информации о клиенте.
* `recvfrom` - Получили отправленные данные
...
Конечно сложно представить проведения отладки высоко нагруженного сетевого приложения, но без данного примера мне казалось 
статья будет не полной.

Получение статистики по syscall
===============================

В качестве бонуса запуск с ключом `-c` который просто выведет статистику по использованию системных вызовов
...
ответа на вопрос "почему не работает?". Надеюсь данная статья будет полезна, и поможет кому нибудь в будущем починить прод
